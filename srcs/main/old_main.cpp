...
int     configure_non_bloking(int fd){
    ...
    ret = fcntl(fd, F_GETFL, O_NONBLOCK);
    return (ret);
}
...
int     init_server_socket(std::string host, int port)
{
    ...
    int sndsize = 1000;
    setsockopt (server_socket, SOL_SOCKET, SO_SNDBUF, &sndsize, sizeof (int));
...
void signal_handler(int signal){
    ...
    for (unsigned int i = 0; i < servers.size(); ++i){
        if (close(servers.at(i).socket) < 0)
            perror("close socket");
    }
    exit(-1);
}
